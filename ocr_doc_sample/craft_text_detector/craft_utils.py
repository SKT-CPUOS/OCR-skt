...
# 避免有些多边形为0，创建np报错，将所有长度都调整为最大的
# def adjustResultCoordinates(polys, ratio_w, ratio_h):
#     adjusted_polys = []
    
#     max_length = max(len(poly) for poly in polys if poly is not None)  # 找到最长的多边形长度
    
#     for poly in polys:
#         if poly is not None and len(poly) > 0:
#             adjusted_poly = np.array(poly) * [ratio_w, ratio_h]
#             # 填充或截断多边形，使其长度与最长的多边形相同
#             adjusted_poly = np.pad(adjusted_poly, ((0, max_length - len(poly)), (0, 0)), mode='constant')
#             adjusted_polys.append(adjusted_poly)

#     # 将所有非空多边形转换为 NumPy 数组
#     adjusted_polys = [poly for poly in adjusted_polys if poly is not None]
#     adjusted_polys = np.array(adjusted_polys)
...
def adjustResultCoordinates(polys, ratio_w, ratio_h):
    ...
    for poly in polys:
        if poly is not None:
            adjusted_poly = np.array(poly) * [ratio_w*2, ratio_h*2] #这里x2，为了将检测框对齐，原本检测框不知道为什么只有一半长度
            adjusted_polys.append(adjusted_poly)
    return np.array(adjusted_polys, dtype=object)